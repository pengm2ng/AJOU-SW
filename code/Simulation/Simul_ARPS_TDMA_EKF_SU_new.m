...
fprintf('simul_ARPS_TDMA_EKF_SU_new\n');
    ...
    for time = 0 : 1 : (50*10^3)/usr_vel
       ...
       %% Estimate Relay Node Positions
        ...
        for ii = 1:NumRel
           ...
           for n = 1 : NumRef
                rel = [];
                rel(1,1) = relxyz(ii,1)+cos(the_rel(ii))*rel_vel*time_slot/NumRef*(ii-1);
                rel(1,2) = relxyz(ii,2)+sin(the_rel(ii))*rel_vel*time_slot/NumRef*(ii-1);
                rel(1,3) = relxyz(ii,3);
                temp_z = genrng(1,rel,refxyz,refid,0,[1 1 0 1 0]);            % measurement value
                Z(n) = temp_z(n);
           end
       ...
       for k = 1:NumRel
        ...
        %% Estimate User position
            ...
            Zu = [];
            for i = 1 : NumRel
                usrxyz(1) = init_usrxyz(1)+cos(theta)*usr_vel*(time+time_slot+slot_num(i)*slot_leng);
                usrxyz(2) = init_usrxyz(2)+sin(theta)*usr_vel*(time+time_slot+slot_num(i)*slot_leng);
                usrxyz(3) = init_usrxyz(3);
                temp_z = genrng(1,usrxyz,relxyz,relid,time,[1 1 0 1 0]);            % measurement value
                Zu(i) = temp_z(i);
            end

            [Xu,Pu] = Extended_KF(f,g,Q,Ru,Zu',Xu,Pu);                     % user positioning using Kalman filter
            estusr = Xu([1 3 5]);
...
% 
%                 [Xu,Pu] = Extended_KF(f,g,Q,Rr,Zr',Xu,Pu);                     % user positioning using Kalman filter
%                 estusr = Xu([1 3 5]);
%             else
%                 estusr = estusr_ini;
%             end
