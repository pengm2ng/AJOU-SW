function [prvec]=genrng(ARNS,usrxyz,svmat,svid,time,esf,...
...
SAerr=0;prnois=0;troperr=0;mperr=0;ionoerr=0;

% Multipath Error generation with standard deviation about 1.4m
mpmat = mpgen(numvis,100000,1);

for N = 1:numvis,
    if ARNS == 2
        pr = norm(svmat(N,:)-usrpos)+avgDiff(N);
    else
...
%      sf=cos(beta);
%      mperr = sf*esf(4)*(mpmat(t1,K)+(tgpssec-t1)*(mpmat(t1+1,K)-mpmat(t1,K)));
% 
%     end,
   end,
   if esf(1)~=0
       % random noise with mean 0 & standard deviation 1
       prnois = rngen(1, 0, 0.1)*0.25;
   end
   if esf(2)~=0,
       % Tropospheric Error model for Pseudolite positioning
%        if usrxyz(3) <= svmat(N,3)
%            troperr = trop_Hopfield(usrxyz, svmat(N, :));
%        else
%            troperr = trop_Hopfield(svmat(N, :), usrxyz);
%        end
       
%        [el, az] = calElAz(svmat(N,:), usrxyz);
       
% SGMP model(Stanford Univ. code¿¡¼­ ¹ßÃé)       
%     El = el/180*pi;
        if usrxyz(3) <= svmat(N,3)
            El = asin((svmat(N,3)-usrxyz(3))/pr);
        else
            El = asin((usrxyz(3)-svmat(N,3))/pr);
        end
        troperr = (2.312 ./ sin(sqrt(El .* El + 1.904e-3)))*0.1 + ...
                   0.084 ./ sin(sqrt(El .* El + 0.6854e-3));
           
% EGNOS model  
%        usrllh = xyz2llh(usrxyz);
%        troperr = esf(2)*trop_EGNOS(usrllh(1),182,el, usrllh(3))

% Hopfield model
%        troperr = esf(2)*tropgen(usrxyz,svmat(N,:));
   end
%    El = e;
   if esf(4)~=0
       % multipath generation with standard deviation about 1.4m(model 1)
        if usrxyz(3) <= svmat(N,3)
            El = asin((svmat(N,3)-usrxyz(3))/pr);
        else
            El = asin((usrxyz(3)-svmat(N,3))/pr);
       ...
       if time == 0
           mperr = mpmat(1,N) * (1-(atan(El)/atan(pi/2)));
       else
           mperr = mpmat(ceil(time),N) * (1-(atan(El)/atan(pi/2)));
       end
   end

% modified by Kyuman(2012.11.25)
   if esf(5)~=0,
       if pr >= 500000
       ionoerr = ...
         esf(5)*ionogen(usrxyz,svmat(N,:),time,freqc,TECmax,TECmin);
       else
           ionoerr = 0;
       end
   end
   
   prvec(N) = pr+SAerr+prnois+troperr+mperr+ionoerr;
end
