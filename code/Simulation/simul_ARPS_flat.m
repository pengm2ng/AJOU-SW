...
%      usrxyz = [(rpt *1000)*cos(2*pi/12) (rpt *1000)*sin(2*pi/12) 0];
    ...
    if cnt ~= 21
    usrxyz_temp = [usrxyz_temp; usrxyz];
    end
    for i = 1 : NumRel
...
%     crel = [CRDia*cos(CAngV*time) CRDia*sin(CAngV*time) relH+randi([1 10])];       
    ...
    switch NumRel
        ...
        case 4
            ...
            relxyz_temp = [relxyz_temp ; rel(4,:), rel(3,:), rel(2,:), rel(1,:)];
        case 5
            
            relxyz = [rel(4,:); rel(3,:); rel(5,:); crel; rel(2,:); rel(1,:)];
            relxyz_temp = [relxyz_temp ; rel(4,:) rel(3,:) rel(5,:) crel rel(2,:) rel(1,:)];
        case 6
...
Total_PositionErr = [Total_PositionErr; Mean_PositionErr];
...
%% 아래에 mat파일로 변수들 저장하는 코드 작성 -> 유의 사항 : 반지름에 따라 저장하는 파일의 이름이 달라야함.
%  총 9개 mat 파일이 생성되어야 함
% 유저의 dop) DOP_usr -> flat_dop30.mat  flat_dop50.mat   flat_dop70.mat으로 저장
%save('flat_dop30.mat','DOP_usr');
% 위치에러) Total_PositionErr -> flat_positionerr30.mat flat_positionerr50.mat flat_positionerr70.mat
%save('flat_positionerr30.mat','Total_PositionErr');
%{
% uav위치 모아 놓은 것) relxyz_temp -> flat_uav_position30.mat flat_uav_position50.mat flat_uav_position70.mat
save('flat_uav_position30.mat','relxyz_temp');
%}
save('truck_position.mat','usrxyz_temp');
%% 코드 작성란 -> if문으로 diameter 변수 크기에 따라 저장파일 이름 바뀌게 하는 것을 추천
