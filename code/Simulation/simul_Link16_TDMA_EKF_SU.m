...
fprintf('simul_Link-16_TDMA_EKF_SU\n');
...
for ii = 1 : 1
    ...
    %% Primary unit Selection
        ...
        while(1)
            ...
            if abs(tmp_dop(cnt2,6))<3 | cnt2 == 2000
                if cnt2 == 2000
    ...
    cnt = 0;
    for time = 1 : 1 : 500%(50*10^3)/su_vel
        cnt = cnt+1;
        ...
        for ii = 1:NumPU
            ...
            for n = 1 : NumGRU
                pu = [];
                pu(1,1) = puxyz(ii,1)+cos(the_pu(ii))*pu_vel*((time-1)+time_slot*gru_slnum(n));
                pu(1,2) = puxyz(ii,2)+sin(the_pu(ii))*pu_vel*((time-1)+time_slot*gru_slnum(n));
                pu(1,3) = puxyz(ii,3);
...
%             pr_ref = [];
%             for m = 1:NumGRU
%                 pr_temp = norm(gruxyz(m,:)-Xu([1 3 5])');
%                 pr_ref = [pr_ref pr_temp];
%             end 
% 
%             index = find(min(pr_ref) == pr_ref);
%             estpu_new = [estpu; gruxyz(index,:)];
            ...
            tmpxyz = [];
            for i = 1 : NumPU
                tmpxyz(i,1) = puxyz(i,1)+cos(the_pu(i))*pu_vel*((time-1)+time_slot*pu_slnum(i));
                tmpxyz(i,2) = puxyz(i,2)+sin(the_pu(i))*pu_vel*((time-1)+time_slot*pu_slnum(i));
