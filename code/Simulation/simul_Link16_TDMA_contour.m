...
for usr_y = pos_start : pos_step : pos_end
    cnt1 = cnt1+1;
    cnt2 = 0;
    for usr_x = pos_start : pos_step : pos_end
        cnt2 = cnt2+1;
        
        usrxyz = [usr_x usr_y 2500];
        ...
        slot_num = randi([1 32],1,NumGRU)-1;
    ...
    %% Estimate Relay Node Position
        ...
        for n = 1:NumPU
            gru = [];
            prvec_rel = genrng(1,puxyz(n,:),gruxyz,gruid,time,[1 1 0 1 0]);
            ...
            temp_pr = genrng(1,puxyz_d(n,:),gru,gruid,dtime,[1 1 0 1 0]);
            pr_pu(k) = temp_pr(k);            
            end
        ...
        %% Calculate pseudo range and Estimate position, Check error
            ...
            prvec = genrng(1,usrxyz,puxyz,puid,time,[1 1 0 1 0]);
        ...
        RMSE_mat(cnt1, cnt2) = sqrt(stdusrxyz(1)^2+stdusrxyz(2)^2+stdusrxyz(3)^2);
    end
...
% file open start
file = 'simul_Link16_TDMA_contour_';
count = 1;
...
fprintf(fd, 'simul_Link16_TDMA_contour_%02d\n', count);
fprintf('simul_Link16_TDMA_contour_%02d is saved !!\n', count);
% file open end
...
for usr_y = 1 : 1 : cnt1
        for usr_x = 1 : 1 : cnt2
        fprintf(fd,'%.10f\t',RMSE_mat(usr_y, usr_x));
        end
    fprintf(fd,'\n');
end
