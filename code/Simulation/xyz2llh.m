...
% change output to array LLA by Juyong Do
...
function llh = xyz2llh(xyz)
	...
	if ((xyz(1) == 0.0) & (xyz(2) == 0.0) & (xyz(3) == 0.0))
        success = 0; lat=NaN; lon=NaN; alt=NaN; return;
% 	        error('WGS xyz at center of earth');
        ...
        for i=1:20
            if ((abs(rhoerror) < 1e-6) && (abs(zerror) < 1e-6)), break; end;
    ...
    llh = [wlat wlon walt];
%     success = ((abs(rhoerror) < 1e-6) && (abs(zerror) < 1e-6));
